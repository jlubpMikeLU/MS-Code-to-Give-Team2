...
async function handleVideoUpload(file) {
    ...
    if (file.size > 100 * 1024 * 1024) {
        alert("File is too large! Please select a video smaller than 100MB.");
        return;
    }

    blockUI();
    document.getElementById("pronunciation_accuracy").innerText = "Processing video...";
    
    // Get the title from the new hidden input field instead of the original_script element
    const homeworkTitle = document.getElementById("homework-title-input").value;
    
    // Check if a title was passed from the homework list
    if (!homeworkTitle) {
        alert("Please select a homework assignment from the Homework List page before uploading a video.");
        unblockUI();
        return;
    }
    ...
    formData.append('title', homeworkTitle); // Use the homework title here
